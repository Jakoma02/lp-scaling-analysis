...
def measure_one(mps_path):
    m = gurobipy.read(mps_path)
    m.Params.Presolve = 0  # Disable presolve so that variable scaling is not changed
    ...
    obj_value = m.objVal if m.Status == gurobipy.GRB.OPTIMAL else None
    duration = end - start
